...
async function blackboxAi(query) {
  try {
    // Tambahkan prompt khusus untuk meminta output HTML
    const htmlPrompt = "buatkan kode HTML lengkap tanpa penjelasan lain, hanya kirimkan kode HTML saja tanpa kata-kata lain atau nama file";
    const fullQuery = `${htmlPrompt} ${query}`;
    ...
    const payload = {
      messages: [{ role: 'user', content: fullQuery, id: '0quFtyH' }],
      id: 'KB5EUHk',
      previewToken: null,
      userId: null,
      codeModelMode: true,
