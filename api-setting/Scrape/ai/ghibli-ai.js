...
async function main() {
  const imageUrl = "https://cloudkuimages.guru/uploads/images/683bf6f5c53d7.jpg";
  const userPrompt = "Convert to ghibli style";

  const result = await ghibli(imageUrl, userPrompt);

  console.log("\n--- HASIL AKHIR PROSES GHIBLI ---");
  if (result && result.success && result.uploadedUrl) {
    console.log("Proses Berhasil!");
    console.log("URL Gambar CloudKu:", result.uploadedUrl);
  } else {
    console.error("Sumber Error:", result?.source || "Tidak diketahui");
    console.error("Pesan Error:", result?.message || "Tidak ada pesan error spesifik.");
    console.error("Detail Error:", result?.details !== undefined ? result.details : "Tidak ada detail.");
  }
  console.log("---------------------------------\n");
}

main().catch(error => {
  console.error("Error tidak tertangani di level atas (main):", error.message);
  if (error.stack) {
